...
namespace RoOvernightTrainLogistics
{
    ...
    class CACGBD
    {
        ...
        public BendersMaster BranchandCut()
        {
            ...
            bdmaster.YCoeff_list = new List<DataStructure>();
            ...
            if (data.stablilization_at_rootnode)
            {
                ...
                Parallel.For(0, total_sets, options, (i, state) =>
                {
                    ...
                    bdmp.BendersDec(lambda_list[i], data.Para_STB_Epsilon, alpha_list[i]);

                    lock (lockObject)
                    {
                        for (int f = 0; f < bdmp.Temp_YCoeff_list.Count; f++)
                        {
                            bdmaster.YCoeff_list.Add(bdmp.Temp_YCoeff_list[f]);
                        }
                    }
                });
            }
            ...
            bdmaster.AddNewValidCuts(bdmaster.YCoeff_list);
        ...
        public void BendersDec()
        {
            ...
            bdmaster.GenBDMasterproblem();
            bdmaster.model.SetParam(Cplex.Param.Threads, 1);
            bdmaster.model.SetParam(Cplex.Param.TimeLimit, availabletime);
            ...
            while (UB - LB > eps * UB && totalrunningtime <= data.TL)
            {
                ...
                bdmaster.model.Solve();
                LB = bdmaster.model.ObjValue;
                mastercost = bdmaster.model.ObjValue;
                eta_value = bdmaster.model.GetValue(bdmaster.omega);
                ...
                {
                    y_solution[j] = 0;
                    double y_j_value = bdmaster.model.GetValue(bdmaster.y_j[j]);
                ...
                {                    
                    ...
                    if (subproblem.model.Solve())
                    {
                        ...
                        //Reset the objective and right hand side of constraints related to y z xi solutions
                        //generating optimality cut
                        IRange bendersoptimalitycut = subproblem.GenBendersCut(bdmaster, worst_u_j);

                        if (bendersoptimalitycut != null)
                        {
                            bdmaster.model.AddCut(bendersoptimalitycut);
                            bdmaster.cutsStore.Add(bendersoptimalitycut);
                        }
                    }
                ...
                {
                    IRange bendersfeasibilitycut = subproblem.GenFeasibilityBendersCut(bdmaster, y_solution);
                    if (bendersfeasibilitycut != null)
                    {
                        bdmaster.model.AddCut(bendersfeasibilitycut);
                        bdmaster.cutsStore.Add(bendersfeasibilitycut);
                    }
            ...
            if (totalSolutionCount >= 1)
            {
                for (int n = 0; n < totalSolutionCount; n++)
                {
                    ...
                    for (int j = 0; j < data.DCSize; j++)
                    {
                        ...
                        eta_val = bdmaster.model.GetValue(bdmaster.omega, n);
                    }
                    double currentobj = 0;
                    for (int j = 0; j < data.DCSize; j++)
                    {
    ...
    class CandCG
    {
        ...
        public void CCGwBAC()
        {
            ...
            while (UB - LB > eps * UB && totalrunningtime <= data.TL)
            {
                ...
                //{
                //    masterproblem.model.AddCut(masterproblem.cutsStore[c]);
                //}
                //masterproblem.cutsStore.Clear();
