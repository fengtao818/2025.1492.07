...
namespace RoOvernightTrainLogistics
{
    ...
    class Data
    {
        ...
        public bool multiCutStrategy = false;//true--using, false-unusing --STG1--        
        public bool paretoCutStrategy = true;//true--using, false-unusing --STG2
        public bool stablilization_at_rootnode = true;//using stabilized strategy at root node--STG3--        
        
        public bool cuttingPlaneStrategy = false;//using cuttling plane at each node (except root node) --STG4--
        public bool mutipleScenarios = false;
        ...
        public Data()
        {
            ...
            demandlist = Program.alldemands.ToList();
    ...
    class Program
    {
        ...
        public static List<double[]> alldemands;
        public static int NumProDistri;//record the maximum number of distribution
        ...
        public static void g_ReadInputData()
        {
            ...
            alldemands = new List<double[]>();
            #endregion

            string[] fileName = { "Input_Nodes", "Input_Links", "Input_Paths", "Input_Demandsrealization", "Input_Disruptions" };
            ...
            for (int s = 0; s < fileName.Length; s++)
            {
                ...
                else if (s == 3)
                {
                    ...
                    while ((strLine = sr.ReadLine()) != null)
                    {
                        ...
                        {
                            double[] demand = new double[g_number_of_nodes];
                            for (int i = 0; i < g_number_of_nodes; i++)
                            {
                                demand[i] = double.Parse(aryLine[i]);
                            }
                            alldemands.Add(demand.ToArray());
        ...
        public static void g_RoMaxFacilityLocation_optimization()
        {
            ...
            g_pFileOutputLog.WriteLine($"{dt.multiCutStrategy}, {dt.paretoCutStrategy}, {dt.stablilization_at_rootnode}");

            g_pFileOutputLog.WriteLine("cplexparaset, solvingprimalsub,BACforworstscenario");
            g_pFileOutputLog.WriteLine($"{dt.cplex_para_set}, {dt.solvingPrimalSub}, {dt.BACforworstscenario}");

            g_pFileOutputLog.WriteLine("ParaCCG_debug_flag, MILPCCG_debug_flag,CCGBAC_debug_flag,CGBD_debug_flag");
            g_pFileOutputLog.WriteLine($"{ParaCCG_debug_flag}, {MILPCCG_debug_flag}, {CCGBAC_debug_flag}, {CGBD_debug_flag}");
