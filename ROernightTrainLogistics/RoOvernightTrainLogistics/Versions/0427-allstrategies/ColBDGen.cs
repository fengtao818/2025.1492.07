...
namespace RoOvernightTrainLogistics
{
    ...
    internal class BendersLazyConsCallback : Cplex.LazyConstraintCallback
    {
        ...
        public override void Main()
        {
            ...
            else
            {
                ...
                if (data.validcut1)
                {
                    ILinearNumExpr validcut_1 = Bendmaster.model.LinearNumExpr();
                    for (int j = 0; j < data.DCSize; j++)
                    {
                        int corCommunityNode = Program.g_DCent_list[j].CorNodeSeqNo;
                        double totalsupply = data.h_i[corCommunityNode];
                        ...
                        validcut_1.AddTerm((1 - worst_u_j[j]) * totalsupply, Bendmaster.y_j[j]);
                    }
                    ...
                    Add(Bendmaster.model.Ge(validcut_1, alldemand));
                }
    ...
    internal class BendersUserCutCallback : Cplex.UserCutCallback
    {
        ...
        public override void Main()
        {
            ...
            IRange bendersoptimalitycut = ccgsub.GenBendersCut(Bendmaster, new int[data.DCSize]);
            if (bendersoptimalitycut != null)
            {
    ...
    class CACGBD
    {
        ...
        public BendersMaster BranchandCut()
        {
            ...
            Program.SolutionIteration.WriteLine("findworsttime, subproblemtime,multicutstime,paretotime,worst_scenario_cost,best objective cost, objective cost, best objective cost");
    ...
    class CandCG
    {
        ...
        public void CCGMILP()
        {
            ...
            while (UB - LB > eps * UB && totalrunningtime <= data.TL)
            {
                ...
                // Find the worst scenario in the context of the current solution
                SCNRMaster submaster = colBDGen.BranchandCutForWorstCsenario(y_solution);
                int[] sol_u_j = new int[data.DCSize];
                double objcost = submaster.model.GetObjValue();
                if (!submaster.feasiblestatus)
                {
                    sol_u_j = submaster.bestFeasibleSolution.ToArray();
                    for (int l = 0; l < data.DCSize; l++)
                    {
                        if (sol_u_j[l] - worst_u_j[l] != 0)
                            Console.WriteLine();
                    }
                }
                else
