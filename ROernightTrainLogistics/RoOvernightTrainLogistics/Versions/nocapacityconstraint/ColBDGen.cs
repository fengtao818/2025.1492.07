...
namespace RoOvernightTrainLogistics
{
    ...
    internal class BendersLazyConsCallback : Cplex.LazyConstraintCallback
    {
        ...
        public override void Main()
        {
            ...
            if (data.LshapedStrategy)
            {                
                double objdiff = worst_scenario_cost - data.LShapedbound;
                
                ILinearNumExpr lshcut = Bendmaster.model.LinearNumExpr();
                lshcut.AddTerm(1, Bendmaster.omega);
                int n = y_solution.ToList().Sum();

                for (int j = 0; j < data.DCSize; j++)
                {
                    if (y_solution[j] == 1)
                    {
                        lshcut.AddTerm(-objdiff, Bendmaster.y_j[j]);
                    }
                    else
                    {
                        lshcut.AddTerm(objdiff, Bendmaster.y_j[j]);
                ...
                Add(Bendmaster.model.Ge(lshcut, (1 - n) * objdiff + data.LShapedbound));
            }
            ...
            if (data.masterlocalbranch)
            {
                if (GetCurrentNodeDepth() == 0 && rootnodefathom)
                {                    
                    double currentObjVal = 0;
                    double bestObjVal = 0;
                    for (int j = 0; j < data.DCSize; j++)
                    {
                        bestObjVal += y_solution[j] * data.f_j[j];
                    }
                    bestObjVal += worst_scenario_cost;

                    int[] bestsol = new int[data.DCSize];
                    bool termination = true;

                    List<int[]> allimpsollist = new List<int[]>();
                    List<int[]> identifiedsol = new List<int[]>();

                    allimpsollist.Add(y_solution);

                    int startTicks = Environment.TickCount;

                    List<int> radiuslist = new List<int>();

                    if(data.masterlocalbranchstrategy)
                    {
                        radiuslist = new List<int> {5, 10};
                    }
                    else
                    {
                        radiuslist = new List<int> {2};
                    }
                    int localloop = 2;

                    int radius = radiuslist.First();
                    ...
                    while (termination)
                    {
                        ...
                        for (int s = 0; s < allimpsollist.Count; s++)
                        {
                            //0-left, 1-right;
                            if (s == allimpsollist.Count - 1)
                            {
                                bdmaster.AddlocalBranchingCons(allimpsollist[s], radius, 0);
                            }
                            else
                            {
                                bdmaster.AddlocalBranchingCons(allimpsollist[s], radius, 1);
                            }
                        }

                        //set time limitaion
                        bdmaster.model.SetParam(Cplex.Param.TimeLimit, 14400);
                        ...
                        bdmaster.model.Use(new localbranchinglazycuts(bdmaster, data));

                        bool solved = bdmaster.model.Solve();

                        if (solved)
                        {                            
                            currentObjVal = bdmaster.model.GetObjValue();
                            
                            if (currentObjVal < bestObjVal)
                            {
                                bestObjVal = currentObjVal;
                                bdmaster.model.ExportModel("bdmasterlocal.lp");
                                bestsol = new int[data.DCSize];

                                for (int j = 0; j < data.DCSize; j++)
                                {
                                    bestsol[j] = (int)bdmaster.model.GetValue(bdmaster.y_j[j]);
                                }
                                allimpsollist.Add(bestsol);
                                identifiedsol.Add(bestsol);
                            }
                            else
                            {
                                if (radiuslist.Count > 0)
                                {
                                    radius = radiuslist.First();
                                    radiuslist.Remove(radius);
                                ...
                                else
                                {
                                    if (localloop <= 1)
                                    {
                                        ...
                                        {
                                            radiuslist = new List<int> { 2, 4, 6, 8 };
                                        }
                                        allimpsollist.Clear();
                                        Random rand = new Random();
                                        int index = rand.Next(0, identifiedsol.Count);
                                        allimpsollist.Add(identifiedsol[index]);
                                        localloop++;
                    ...
                    while (termination)
                    {
                        ...
                        //Step 2: determine theta and add cutoff constraint
                        bdmaster.AddcutoffCons(bestObjVal, theta);

                        //Step 3: reset objective function as Hamming distance
                        IObjective objExpr = bdmaster.model.GetObjective();
                        objExpr.ClearExpr();

                        int constant = 0;
                        ILinearNumExpr consexpr = bdmaster.model.LinearNumExpr();
                        for (int j = 0; j < data.DCSize; j++)
                        {
                            if (bestsol[j] == 1)
                            {
                                consexpr.AddTerm(-1, bdmaster.y_j[j]);
                                constant++;
                            }
                            else
                            {
                        ...
                        objExpr.Expr = bdmaster.model.Sum(consexpr, constant);
                        ...
                        if (bdmaster.model.Solve())
                        {
                            //bdmaster.model.ExportModel("localbranchingmodel.lp");
                            currentObjVal = bdmaster.model.GetObjValue();
                            if (currentObjVal < bestObjVal)
                            {
                                bestObjVal = currentObjVal;
                    ...
                    for (int sl = 0; sl < identifiedsol.Count; sl++)
                    {
                        int[] cur_sol = identifiedsol[sl];
                        CACGBD colBDGen = new CACGBD();
                        SCNRMaster submaster = colBDGen.BranchandCutForWorstCsenario(cur_sol);
                        //stg 2: getting worst scenario by branching and cut
                        if (data.solvingPrimalSub)
                        {
                            findw0rstscenario.Start();
                            //worst_xi_j = colBDGen.BranchandCutForWorstCase(y_solution, z_solution);
                            ...
                            worst_scenario_cost = colBDGen.scenarioCost;
                        }

                        //Reset the objective and right hand side of constraints related to y z xi solutions
                        ccgsub.ResetOBJaRHS_XI(cur_sol, worst_u_j);
                        ccgsub.model.Solve();
                        //ccgsub.model.ExportModel("ccgsubtestmodel.lp");
                        //generating optimality cut
                        bendersoptimalitycut = ccgsub.GenBendersCut(Bendmaster, worst_u_j);
                        if (bendersoptimalitycut != null)
                        {
                            Add(bendersoptimalitycut);
                            Bendmaster.cutsStore.Add(bendersoptimalitycut);
    ...
    internal class localbranchinglazycuts : Cplex.LazyConstraintCallback
    {
        internal BendersMaster Bendmaster;
        internal Data data;
        internal int count;
        //internal function
        internal CCGSub ccgsub;
        internal DualCCGSub dualsubproblem;
        //internal BendersDualSub bddsub;
        /// <summary>
        /// initialize BendersLazyConsCallback
        /// </summary>
        /// <param name="BDmaster"></param master problem with y and z>
        /// <param name="dt"></param the set of initial benders cuts>
        public localbranchinglazycuts(BendersMaster BDmaster, Data dt)
        {
            Bendmaster = BDmaster;
            data = dt;
            count = 0;
            ...
            ccgsub.GenCCGSubproblem(new int[data.DCSize], new int[data.DCSize]);
            //bddsub = new BendersDualSub();
            //bddsub.GenBendersubDual(new int[data.DCSize], new double[data.DCSize], new int[data.DCSize]);
    ...
    internal class BendersHeuristicCallback : Cplex.HeuristicCallback
    {
        internal BendersMaster Bendmaster;
        internal Data data;
        CCGSub ccgsub;
        public BendersHeuristicCallback(BendersMaster Bendmaster, Data data)
        {
        ...
        public override void Main()
        {
            ...
            y_sol = GetValues(Bendmaster.y_j);

            List<double> thresholdlist = new List<double>();
            for (int j = 0; j < data.DCSize; j++)
            {
                if (y_sol[j] < 1 && y_sol[j] > 0)
                {
                    thresholdlist.Add(y_sol[j]);
                }
            }
            thresholdlist.Sort();
            double objvalue = GetObjValue();
            double omegaval = GetValue(Bendmaster.omega);

            double[] incombentY = new double[data.DCSize];

            for (int l = 0; l < thresholdlist.Count; l++)
            {
                int[] newroundY = new int[data.DCSize];
                double threshold = thresholdlist[l];
                double firstterm = 0;
                for (int j = 0; j < data.DCSize; j++)
                {
                    if (y_sol[j] >= threshold)
                    {
                        newroundY[j] = 1;
                        firstterm += data.f_j[j];
                    }
                    else
                    {
                        newroundY[j] = 0;
                ...
                CACGBD colBDGen = new CACGBD();
                SCNRMaster submaster = colBDGen.BranchandCutForWorstCsenario(newroundY);
                int[] worst_u_j = new int[data.DCSize];

                for (int j = 0; j < data.DCSize; j++)
                {
                    var u_val = submaster.model.GetValue(submaster.u_j[j]);
                ...
                ccgsub.ResetOBJaRHS_XI(newroundY, worst_u_j);
                ...
                ccgsub.model.Solve();
                double totalcost = firstterm + ccgsub.model.GetObjValue();

                if (ccgsub.model.GetObjValue() > omegaval)
                {
                    objvalue = totalcost;
                    omegaval = ccgsub.model.GetObjValue();

                    for (int j = 0; j < data.DCSize; j++)
                    {
                        incombentY[j] = newroundY[j];
                    }                    
                }
            }
            SetSolution(Bendmaster.y_j, incombentY, objvalue);            
            #endregion
        }
    ...
    internal class SCNRLazyConsCallback : Cplex.LazyConstraintCallback
    {
        ...
        public override void Main()
        {
            ...
            if (data.sublocalbranch)
            {
                if (GetCurrentNodeDepth() == 0 && rootnodefathom)
                {
                    bool impsol = true;
                    double currentObjVal = 0;
                    double bestObjVal = GetObjValue();

                    int[] bestsol = new int[data.DCSize];
                    bool termination = true;

                    List<int[]> allImpSollist = new List<int[]>();//save all sols include current incumbent
                    List<int[]> identifiedsol = new List<int[]>();

                    if (u_sol.ToList().Sum() == 0)
                    {
                        int counter = 0;
                        for (int j = 0; j < data.DCSize; j++)
                        {
                            ...
                            if (counter >= data.max_dstroyed_DCs)
                                break;
                        }
                        identifiedsol.Add(u_sol);

                        modifiedSub.ResetOBJaRHS_XI(y_solution, u_sol, data.big_M_r);
                        issolved = modifiedSub.model.Solve();
                        bestObjVal = modifiedSub.model.GetObjValue();
                    }
                    allImpSollist.Add(u_sol);
                    ...
                    int startTicks = Environment.TickCount;
                    int radius = 2;//not less than the max destroyed DCs

                    while (termination)
                    {
                        //local branching search
                        DualCCGSub duaccgsub = new DualCCGSub();
                        //generate benders master problem
                        duaccgsub.GenDualofCCGsub(y_solution, deltaval);
                        ...
                        for (int s = 0; s < allImpSollist.Count; s++)
                        {
                            if (impsol)
                            {
                                ...
                                if (s == allImpSollist.Count - 1)
                                {
                                    duaccgsub.AddlocalBranchingCons(allImpSollist[s], radius, 0);
                                }
                                else
                                {
                                    if (radius > y_solution.ToList().Sum() - data.max_dstroyed_DCs)
                                    {
                                        radius = y_solution.ToList().Sum() - data.max_dstroyed_DCs;
                                    }
                                    duaccgsub.AddlocalBranchingCons(allImpSollist[s], radius, 1);
                                }
                            }
                            else
                            {
                                //1-right, no improved solution then stop branching
                                duaccgsub.AddlocalBranchingCons(allImpSollist[s], radius, 1);
                                termination = false;
                            }
                        }
                        ...
                        //set time limitaion
                        duaccgsub.model.SetParam(Cplex.Param.TimeLimit, 5);
                        bool solved = duaccgsub.model.Solve();
                        if (solved)
                        {
                            //bdmaster.model.ExportModel("localbranchingmodel.lp");
                            currentObjVal = duaccgsub.model.GetObjValue();
                            if (currentObjVal > bestObjVal)
                            {
                                bestObjVal = currentObjVal;
                                ...
                                for (int j = 0; j < data.DCSize; j++)
                                {
                                    bestsol[j] = (int)duaccgsub.model.GetValue(duaccgsub.u_j[j]);
                                }
                                allImpSollist.Add(bestsol);
                                identifiedsol.Add(bestsol);
                            }
                        ...
                        {
                            duaccgsub.model.ExportModel("duaccgsubmodel.lp");
                        }
                        int endTicks = Environment.TickCount;
                        if (endTicks - startTicks > 5000)
                        {
                    ...
                    for (int sl = 0; sl < identifiedsol.Count; sl++)
                    {
                        ...
                        modifiedSub.ResetOBJaRHS_XI(y_solution, cur_sol, data.big_M_r);
                        issolved = modifiedSub.model.Solve();
    ...
    class KKT
    {
        ...
        /// used for the valid cut for ccg master problem
        /// </summary>        
        public void GenKKTSubproblem(int[] y_sol)
        {
            #region ccg subproblem model           
            ...
            INumVar[] deltaindicator_r = new INumVar[data.pathSize];
            INumVar[] vindicator_i = new INumVar[data.nodeSize];
            INumVar[] windicator_j = new INumVar[data.DCSize];
            INumVar[] xindicator_r = new INumVar[data.pathSize];
            INumVar[] zindicator_j = new INumVar[data.DCSize];

            for (int r = 0; r < data.pathSize; r++)
            {
                delta_r[r] = model.NumVar(0, float.MaxValue, NumVarType.Float, $"delta_{r}");
                number_of_var++;
                deltaindicator_r[r] = model.BoolVar($"delind_{r}");
                number_of_var++;
            }
            for (int i = 0; i < data.nodeSize; i++)
            {
                ...
                number_of_var++;
                vindicator_i[i] = model.BoolVar($"vind_{i}");
                number_of_var++;
            }
            for (int j = 0; j < data.DCSize; j++)
            {
                ...
                number_of_var++;
                windicator_j[j] = model.BoolVar($"wind_{j}");
                number_of_var++;
            ...
            for (int r = 0; r < data.pathSize; r++)
            {
                x_r[r] = model.NumVar(0, float.MaxValue, NumVarType.Float, $"x_{r}");
                number_of_var++;
                xindicator_r[r] = model.BoolVar($"xind_{r}");
                number_of_var++;
            }
            ...
            for (int j = 0; j < data.DCSize; j++)
            {
                z_j[j] = model.NumVar(0, float.MaxValue, NumVarType.Float, $"z_{j}");
                number_of_var++;
                zindicator_j[j] = model.BoolVar($"zind_{j}");
                number_of_var++;
            }
            ...
            if (cons_1 == 1)
            {
                for (int r = 0; r < data.pathSize; r++)
                {
                    ...
                    cons.AddTerm(-y_sol[data.s_r[r]], u_j[data.s_r[r]]);

                    IRange constraint = model.AddGe(cons, -y_sol[data.s_r[r]], $"Transport_capacity_{r}_{data.s_r[r]}");
                    relatedtodual_Delta_value[r] = constraint;
                }
            }
            ...
            if (cons_4 == 1)
            {
                ...
                for (int r = 0; r < data.pathSize; r++)
                {
                    ...
                    for (int j = 0; j < data.DCSize; j++)
                    {
                        consexpr.AddTerm(-data.h_i[data.e_r[r]] * data.pathtoStartDC[r, j], w_j[j]);
                    }
                    for (int i = 0; i < data.nodeSize; i++)
                    {
            ...
            //complementary slackness condition-primal
            int cons_6 = 1;
            if (cons_6 == 1)
            {
                for (int r = 0; r < data.pathSize; r++)
                {
                    ...
                    ILinearNumExpr consexpr = model.LinearNumExpr();
                    consexpr.AddTerm(-data.big_M_r[r], deltaindicator_r[r]);

                    model.AddLe(model.Sum(cons, y_sol[data.s_r[r]]), model.Sum(data.big_M_r[r], consexpr), $"TC_CSC1_{r}_{data.s_r[r]}");

                    ILinearNumExpr consexpr1 = model.LinearNumExpr();
                    consexpr1.AddTerm(data.big_M_r[r], deltaindicator_r[r]);
                    model.AddLe(delta_r[r], consexpr1, $"TC_CSC2_{r}_{data.s_r[r]}");
                }
            }
            ...
            if (cons_7 == 1)
            {
                for (int i = 0; i < data.nodeSize; i++)
                {
                    ...
                    ILinearNumExpr consexpr = model.LinearNumExpr();
                    consexpr.AddTerm(-data.B_i[i], vindicator_i[i]);
                    //contraint.AddTerm(1, s_i[i]);
                    model.AddLe(model.Sum(contraint, -1), model.Sum(data.B_i[i], consexpr), $"FD_CSC_{i}");

                    ILinearNumExpr consexpr1 = model.LinearNumExpr();
                    consexpr1.AddTerm(data.B_i[i], vindicator_i[i]);
                    model.AddLe(v_i[i], consexpr1, $"TD_CSC2_{i}");
                    number_of_con++;
                }
            }
            int cons_8 = 1;
            if (cons_8 == 1)
            {
                for (int j = 0; j < data.DCSize; j++)
                {
                    ...
                    contraint.AddTerm(1, z_j[j]);
                    ILinearNumExpr consexpr = model.LinearNumExpr();
                    consexpr.AddTerm(-data.big_M*10, windicator_j[j]);

                    model.AddLe(contraint, model.Sum(data.big_M * 10, consexpr), $"PD_CSC_{j}");

                    ILinearNumExpr consexpr1 = model.LinearNumExpr();
                    consexpr1.AddTerm(data.big_M * 10, windicator_j[j]);
                    model.AddLe(w_j[j], consexpr1, $"PD_CSC2_{j}");
                    number_of_con++;
                }
            }
            //complementary slackness condition-dual
            int cons_9 = 1;
            if (cons_9 == 1)
            {
                ILinearNumExpr consexpr = model.LinearNumExpr();
                for (int r = 0; r < data.pathSize; r++)
                {
                    ...
                    ILinearNumExpr cons = model.LinearNumExpr();
                    cons.AddTerm(-data.big_M_r[r], xindicator_r[r]);

                    model.AddLe(model.Sum(consexpr, data.d_r[r] * data.h_i[data.e_r[r]]), model.Sum(data.big_M_r[r], cons), $"X_csc[{r}]");

                    ILinearNumExpr cons1 = model.LinearNumExpr();
                    cons1.AddTerm(data.big_M_r[r], xindicator_r[r]);
                    model.AddLe(x_r[r], cons1, $"X_CSC2_{r}");
                    number_of_con++;
                }
            }
            ...
            if (cons_10 == 1)
            {
                for (int j = 0; j < data.DCSize; j++)
                {
                    ...
                    ILinearNumExpr cons = model.LinearNumExpr();
                    cons.AddTerm(-1, w_j[j]);
                    model.AddLe(model.Sum(cons, data.c_j[j]), model.Sum(data.big_M * 10, consexpr), $"Z_Dual_{j}");

                    ILinearNumExpr consexpr1 = model.LinearNumExpr();
                    consexpr1.AddTerm(data.big_M * 10, zindicator_j[j]);
                    model.AddLe(z_j[j], consexpr1, $"PD_CSC2_{j}");
                    number_of_con++;
                }
            }
            int cons_11 = 1;
    ...
    class CCGSub
    {
        ...
        {
            #region linear ccg subproblem model           
        ...
        public SCNRMaster BranchandCutForWorstCsenario(int[] y_sol)
        {
            ...
            if (data.localBranchStrategy)
            {
                ...
                for (int j = 0; j < data.DCSize; j++)
                {
                    if (y_sol[j] == 1)
                    {
                        //find all possible routes with their cost
                        List<double> costlist = new List<double>();
                        for (int r = 0; r < data.pathSize; r++)
                        {
                            if(data.s_r[r] == j)
    ...
    class CandCG
    {
        ...
        public void ParallelCCG()
        {
            ...
            masterproblem.model.SetParam(Cplex.Param.TimeLimit, data.TL);
        ...
        public void CCGMILP()
        {
            ...
            if (cur_status == Cplex.Status.Optimal)
                feasible_flag = true;
