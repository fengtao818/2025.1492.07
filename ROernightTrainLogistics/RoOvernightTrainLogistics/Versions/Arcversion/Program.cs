...
namespace RoOvernightTrainLogistics
{
    ...
    class Data
    {
        ...
        public int Min_open_DCs;//used in inequality
        public int max_open_DCs;
        ...
        public bool paretoCutStrategy = true;//true--using, false-unusing --STG2--
        public bool cplex_para_set = true;//set aggressive parameter to accelerate solution process --STG3--         
        public bool stablilization_at_rootnode = true;//using stabilized strategy at root node--STG4--
        public bool cuttingPlaneStrategy = true;//using cuttling plan at each node (except root node) --STG6--
        public bool solvingPrimalSub = true;////true--solving, false-unsolving--STG5--

        public bool solvingprimalfordual = true;//whether or not to obtain dual values by solving primal problem
        public bool nogoodcuts = false;//whether or not to obtain dual values by solving primal problem
        public int maxSubMultiCuts = 2;
    ...
    class Solution
    {
        ...
        //    for (int j = 0; j < m_number_of_DCents; j++)
        //    {
        //        z_solution[j] = master.model.GetValue(master.z_j[j]);
        ...
        //    ColBDGen cobdgen = new ColBDGen();

        //    int[] xi_sol = cobdgen.BranchandCutForWorstCsenario(y_solution, z_solution);

        //    CCGSubproblem subproblem = new CCGSubproblem();

        //    subproblem.GeneCCGSubproblem(y_solution, z_solution, xi_sol);
    ...
    class Program
    {
                ...
                else if (s == 1)
                {
                    ...
                    while ((strLine = sr.ReadLine()) != null)
                    {
                        ...
                        else
                        {
                            ...
                            sectionID = int.Parse(aryLine[8]); direct = aryLine[9];
        ...
        {
            #region 调用 CCG & CG 求解
            Stopwatch optimize_CCG = new Stopwatch();
            Stopwatch optimize_BDD = new Stopwatch();
            Stopwatch optimize_CGBD = new Stopwatch();
            ...
            if (MILPCCG_debug_flag == 1)
            {
                ...
                optimize_CCG.Stop();
                total_CCG = optimize_CCG.ElapsedMilliseconds;
            }

            ////stage 3 : column and Benders cut optimization
            ...
            //    Solution solution = new Solution();
            //    ColBDGen colBDGen = new ColBDGen();

            //    BendersMaster Bendersmaster = colBDGen.BranchandCut();                
            //    optimize_CGBD.Stop();
            //    total_BDD = optimize_CGBD.ElapsedMilliseconds;
            ...
            Console.WriteLine("CPU Running Time of CCG = {0} seconds", total_CCG / unit);
            Console.WriteLine("CPU Running Time of BDD = {0} seconds", total_BDD / unit);

            g_pFileOutputLog.Write("CPU Running Time of CCG =,{0}, seconds\r\n", total_CCG / unit);
            g_pFileOutputLog.Write("CPU Running Time of BDD =,{0}, seconds\r\n", total_BDD / unit);

            g_pFileOutputLog.WriteLine("STG1-multicuts, STG2-Paretocut, STG3-Bestbound,STG4-Stabilization,STG5-solvingprimal,STG6-cuttingplane");
            g_pFileOutputLog.WriteLine($"{dt.multiCutStrategy}, {dt.paretoCutStrategy}, {dt.cplex_para_set},{dt.stablilization_at_rootnode}," +
                $"{dt.solvingPrimalSub},{dt.cuttingPlaneStrategy}");
            #endregion
        }
