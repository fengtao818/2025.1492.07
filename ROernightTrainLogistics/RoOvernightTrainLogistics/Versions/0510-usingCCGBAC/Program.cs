...
namespace RoOvernightTrainLogistics
{
    ...
    class Data
    {
        ...
        public bool validcut2 = true;//true--using, false-unusing --STG2
        ...
        public bool paretoCutStrategy = false;//
        ...
        public Data()
        {
            ...
            for (int i = 0; i < nodeSize; i++)
            {
                B_i[i] = (maxtransportcost + maxpurchasecost/* + maxopencost*/) * h_i[i];
            }
            for (int i = 0; i < nodeSize; i++)
            {
    ...
    class Solution
    {
        ...
        public void output_solution(BendersMaster master)
        {
            ...
            Program.solution_output.Write("y_sol" + ",");
            double fixedcost = 0;
            double omega_val = master.model.GetValue(master.omega);
            for (int j = 0; j < m_number_of_DCents; j++)
            {
            ...
            Program.solution_output.Write("fixed cot:{0}, subcost: {1}", fixedcost, omega_val);
    ...
    class Program
    {
        ...
        public static void g_RoMaxFacilityLocation_optimization()
        {
            ...
            g_pFileOutputLog.WriteLine("STG1-multicuts, STG2-validcut, STG3-Localbranch,STG4-Stabilization");
            g_pFileOutputLog.WriteLine($"{dt.multiCutStrategy}, {dt.validcut2}, {dt.localBranchStrategy},{dt.stablilization_at_rootnode}");

            g_pFileOutputLog.WriteLine("cplexparaset, solvingprimalsub,BACforworstscenario,MultiplescenariosforCCG");
            g_pFileOutputLog.WriteLine($"{dt.cplex_para_set}, {dt.solvingPrimalSub}, {dt.BACforworstscenario},{dt.MultiplescenariosforCCG}");
