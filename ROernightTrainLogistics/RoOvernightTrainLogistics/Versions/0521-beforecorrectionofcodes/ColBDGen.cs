...
namespace RoOvernightTrainLogistics
{
        ...
        public override void Main()
        {
            ...
            if (issubprofeasi)
            {
                ...
                if(omega < worst_scenario_cost)
                {
                    ...
                    if (data.validcut2)
                    {
                        ILinearNumExpr validcut_2 = Bendmaster.model.LinearNumExpr();
                        for (int j = 0; j < data.DCSize; j++)
                        {
                            int corCommunityNode = Program.g_DCent_list[j].CorNodeSeqNo;
                            double totalsupply = data.h_i[corCommunityNode];
                            ...
                            for (int r = 0; r < data.pathSize; r++)
                            {
                                if (data.s_r[r] == j)
                                {
                                    if (data.d_r[r] > allocationcost)
                                    {
                                        allocationcost = data.d_r[r];
                                    }
                                }
                            }
                            validcut_2.AddTerm((1 - worst_u_j[j]) * totalsupply * (data.c_j[j] + allocationcost), Bendmaster.y_j[j]);

                        }
                        validcut_2.AddTerm(-1, Bendmaster.omega);
                        Add(Bendmaster.model.Ge(validcut_2, 0));
                    }
                }
    ...
    internal class BLCCforCCGMaster : Cplex.LazyConstraintCallback
    {
        ...
        /// <param name="BDmaster"></param master problem with y and z>
        /// <param name="dt"></param the set of initial benders cuts>
        public BLCCforCCGMaster(CCGMaster CCGmaster, Data dt, List<int[]> multiscenario)
        {
            this.CCGmaster = CCGmaster;
            data = dt;
            ...
            //for (int c = 0; c < CCGmaster.cutsStore.Count; c++)
            //{
            //    Add(CCGmaster.cutsStore[c]);
            //}
        }
        public override void Main()
        {
            ...
            int[] worst_u_j = new int[data.DCSize];
            int[] y_solution = new int[data.DCSize];
            double omegaval = GetValue(CCGmaster.omega);
            ...
            {
                //worst_xi_j[j] = 0;
                var yvalue = GetValue(CCGmaster.y_j[j]);
                if (yvalue > 0.5)
                {
                    y_solution[j] = 1;
                    numberofopens++;
            ...
            for (int s = 0; s < multicutslist.Count; s++)
            {
                ...
                for (int r = 0; r < data.pathSize; r++)
                {
                    ccgsub.relatedtodual_Delta_value[r].UB = y_solution[data.s_r[r]] * (1 - scenario[data.s_r[r]]);
                }


                if (ccgsub.model.Solve())
                {
                    worst_scenario_cost = ccgsub.model.GetObjValue();
                    if (ccgsub.model.GetObjValue() - omegaval > 1e-6 * omegaval)
                    {
                        worst_scenario_cost = ccgsub.model.GetObjValue();
                        worst_u_j = scenario.ToArray();

                        IRange optimalitycut = ccgsub.GenBendersCutforCCG(CCGmaster, scenario);

                        if (optimalitycut != null)
                        {
                            Add(optimalitycut);
                            CCGmaster.cutsStore.Add(optimalitycut);
                        }
                    }
                ...
                {
                    IRange feasicut = ccgsub.GenFeasCCGCut(CCGmaster, y_solution);

                    if (feasicut != null)
                    {
                        Add(feasicut);
                        CCGmaster.feasiblecutsStore.Add(feasicut);
                    }
                    break;
                }
            ...
            if (data.paretoCutStrategy)
            {
                BendersDualSub pareto = new BendersDualSub();
                pareto.GenBendersubDual(y_solution, worst_u_j);
                IRange paretoCut = pareto.GenParetoCutforCCGMaster(y_solution, worst_u_j, worst_scenario_cost, data.pareto_y_sol, data.pareto_u_sol, CCGmaster);

                if (paretoCut != null)
                {
                    Add(paretoCut);
                    CCGmaster.cutsStore.Add(paretoCut);
                }
                pareto.model.End(); pareto = null;
            }
    ...
    internal class BendersforrootofCCGMaster : Cplex.LazyConstraintCallback
    {
        internal CCGMaster CCGmaster;
        internal Data data;
        internal int count;
        ...
        /// <param name="BDmaster"></param master problem with y and z>
        /// <param name="dt"></param the set of initial benders cuts>
        public BendersforrootofCCGMaster(CCGMaster CCGmaster, Data dt)
        {
            this.CCGmaster = CCGmaster;
            data = dt;
            multicutslist = new List<int[]>();
            count = 0;
            //generate a new CCG subproblem
            ccgsub = new CCGSub();
            ccgsub.GenCCGSubproblem(new int[data.DCSize], new int[data.DCSize]);
        ...
        public override void Main()
        {
            ...
            if (issubprofeasi)
            {
                ...
                if (omegaval < worst_scenario_cost)
                {
                    //generating optimality cut
                    IRange bendersoptimalitycut = ccgsub.GenBendersCutforCCG(CCGmaster, worst_u_j);

                    if (bendersoptimalitycut != null)
                    {
                        Add(bendersoptimalitycut);
                        CCGmaster.cutsStore.Add(bendersoptimalitycut);
                    }
                    subproblemtime.Stop();
                    subproblemtimerecord += subproblemtime.ElapsedMilliseconds;
                    ...
                    if (data.multiCutStrategy)
                    {
                        while (multicutslist.Count != 0)
                        {
                            ...
                            if (solveflag)
                            {
                                IRange validcut = ccgsub.GenBendersCutforCCG(CCGmaster, scenario);
                                double subobj = ccgsub.model.GetObjValue();

                                if (validcut != null)
                                {
                                    Add(validcut);
                                    CCGmaster.cutsStore.Add(validcut);
                                }
                            }
                        }
                    }
            ...
            else
            {
                ...
                IRange bendersfeasibilitycut = ccgsub.GenFeasCCGCut(CCGmaster, y_solution);
                if (bendersfeasibilitycut != null)
                {
                    Add(bendersfeasibilitycut);
                    CCGmaster.feasiblecutsStore.Add(bendersfeasibilitycut);
                }
    ...
    internal class BendersUserCutCallback : Cplex.UserCutCallback
    {
        ...
        public BendersUserCutCallback(BendersMaster BDmaster, Data dt)
        {
            Bendmaster = BDmaster;
            data = dt;
            Iteration = 0;
        ...
        public override void Main()
        {
            ...
            IRange bendersoptimalitycut = ccgsub.GenLinearBendersCut(Bendmaster, new int[data.DCSize]);
            if (bendersoptimalitycut != null)
            {
    ...
    /// invoked at node which finds a linear solution
    /// </summary>
    internal class BLCCUserCutforCCGMaster : Cplex.UserCutCallback
    {
        ...
        internal CCGSub ccgsub;
        internal int Iteration;
        public BLCCUserCutforCCGMaster(CCGMaster BDmaster, Data dt)
        {
            Bendmaster = BDmaster;
            data = dt;
            Iteration = 0;
    ...
    internal class BendersHeuristicCallback : Cplex.HeuristicCallback
    {
        ...
        public override void Main()
        {
            ...
            if (!heuck)
            {
                ...
                List<double> creasingorderlist = new List<double>();

                for (int j = 0; j < data.DCSize; j++)
                {
                    if (y_sol[j] > 0)
                    {
                        creasingorderlist.Add(y_sol[j]);
                    }
                }

                creasingorderlist.Sort();

                int[] final_y_sol = new int[data.DCSize];
                double eps_threshold = data.Para_Location_fixthreshold;
                double[] output_y_sol = new double[data.DCSize];
                //save the dcenters NO with value larger than the threshold
                List<int> selectedFacilites = new List<int>();
                double thresholds = 0;

                while (true)
                {
                    thresholds = creasingorderlist[0];
                    final_y_sol = new int[data.DCSize];
                    output_y_sol = new double[data.DCSize];

                    //in case the constraint on budget is unsatisfied

                    //the destroyed action takes place in case yj = 1
                    for (int j = 0; j < data.DCSize; j++)
                    {
                        if (y_sol[j] >= thresholds)
                        {
                            final_y_sol[j] = 1;
                            output_y_sol[j] = 1;
                        }
                    ...
                    //determine the upper bound of delta in case of ysolution
                    bddsub.ResetBDDSubObjforDelta(final_y_sol, final_y_sol);
                    bddsub.model.Solve();
                    ...
                    //update latest y and delta
                    dccgsub.ResetdualccgSubObjcons(final_y_sol, deltaval);
                    bool issolved = dccgsub.model.Solve();
                    ...
                    ccgsub.ResetOBJaRHS_XI(final_y_sol, worst_u_j);
                    issubprofeasi = ccgsub.model.Solve();
                    
                    if (issubprofeasi )
                    {

                        double worst_scenario_cost = ccgsub.model.GetObjValue();
                        if (omega < worst_scenario_cost)
                        {
                            //generating optimality cut
                            IRange bendersoptimalitycut = ccgsub.GenBendersCut(Bendmaster, worst_u_j);
                            ...
                            if (bendersoptimalitycut != null)
                            {
                                Bendmaster.model.AddLazyConstraint(bendersoptimalitycut);
                                Bendmaster.cutsStore.Add(bendersoptimalitycut);
                            }

                            //SetSolution(Bendmaster.y_j, output_y_sol);
                            break;
                        }
                        
                    }

                    creasingorderlist.RemoveAt(0);
                    if (creasingorderlist.Count < data.max_dstroyed_DCs + 1)
                        break;
                }
            }
    ...
    class BendersMaster
    {
        ...
        public void GenBDMasterproblem()
        {
            ...
            omega = model.NumVar(/*-float.MaxValue*/0, float.MaxValue, NumVarType.Float, "omega");
            for (int j = 0; j < data.DCSize; j++)
            {
        ...
        public void BendersDec(double lambda, double eps_threshod, double alpha)
        {
            ...
            double solvetime = 0;
            while (true && solvetime<data.rootnodesolvetime)
            {
                iter++;
                y_sol = new int[data.DCSize];
                if (iter != 0)
                {
                    for (int j = 0; j < data.DCSize; j++)
                    {
                        //worst_xi_j[j] = 0;
                       var yvalue = model.GetValue(y_j[j]);
                ...
                ccgsub = new CCGSub();
                ccgsub.GenCCGSubproblem(y_sol, u_sol);
                ccgsub.model.SetParam(Cplex.Param.Preprocessing.Presolve, false);
                ...
                if (primalstatus == Cplex.Status.Optimal)
                {
                    ...
                    double[] pi_a_values = new double[data.linkSize];//>0
                    ...
                    for (int a = 0; a < data.linkSize; a++)
                    {
                        pi_a_values[a] = ccgsub.model.GetDual(ccgsub.relatedtodual_Pi_value[a]);
                ...
                iteration++;
                solvetime = (Environment.TickCount - tickcountstart)/1000;
            }
            #endregion
        }
        /// <summary>
        ...
        public void StabilizedBendersDec(double lambda, double eps_threshod, double alpha)
        {
            ...
            while (true)
            {
                ...
                if (primalstatus == Cplex.Status.Optimal)
                {
                    ...
                    for (int a = 0; a < data.linkSize; a++)
                    {
                        //pi_a_values[a] = model.GetDual(ccgsub.relatedtodual_Pi_value[a]);
                        sumofv += pi_a_values[a] * data.linklist[a].LinkCapacity;
                    }
    ...
    class CCGSub
    {
        ...
        public IRange GenFeasibilityBendersCut(BendersMaster BDM, int[] y_sol)
        {
            ...
            //IRange cut = BDM.model.Le(extrem_point_exp, -sumofv);
        ...
        /// <param name="xi_sol"></param worst scenario>
        /// <returns></returns>
        public IRange GenFeasCCGCut(CCGMaster BDM, int[] y_sol)
        {
            #region
        ...
        /// <param name="xi_sol"></param worst scenario>
        /// <returns></returns>
        public IRange GenLinearBendersCut(BendersMaster BDM, int[] u_sol)
        {
            #region
            double[] v_i_values = new double[data.nodeSize];//>0
            double[] delta_r_values = new double[data.pathSize];//>0
        ...
        /// <param name="xi_sol"></param worst scenario>
        /// <returns></returns>
        public IRange GenLinearBendersCut(CCGMaster BDM, int[] u_sol)
        {
            #region
            double[] v_i_values = new double[data.nodeSize];//>0
            double[] delta_r_values = new double[data.pathSize];//>0
    ...
    class CACGBD
    {
        ...
        public void generateParetoPoint()
        {
            ...
            //for (int j = 0; j < data.max_dstroyed_DCs + 1; j++)
            //{
            //    u_sol[j] = 1;
            //}
            //ccgmst.u_l_j.Add(u_sol);
        ...
        public void generatesubsetvalidcuts(BendersMaster Bendmaster)
        {
            #region
            int[] ysol = new int[data.DCSize];
            Random rand = new Random();
            ...
            int dcinable = 3;
            while ((Environment.TickCount - starttickcount)/1000 < data.rootnodesolvetime)
            {
                int count = 0;

                while (count < dcinable)
                {
                    int[] tem_ysol = ysol;
                    List<int> dcindex = ysol.ToList();

                    int ind = rand.Next(0, dcindex.Count);
                    tem_ysol[dcindex[ind]] = 0;
                    dcindex.RemoveAt(ind);
                    count++;

                    CACGBD colBDGen = new CACGBD();
                    SCNRMaster submaster = colBDGen.BranchandCutForWorstCsenario(tem_ysol);
                    int[] worst_u_j = new int[data.DCSize];

                    // Find the worst scenario by BAC in the context of the current solution
                    bool issubprofeasi = submaster.feasiblestatus;
                    ...
                    if (issubprofeasi)
                    {
                        ...
                        validcut_2.AddTerm(-1, Bendmaster.omega);
                        Bendmaster.model.AddGe(validcut_2, 0);
                    }
                }
        ...
        public BendersMaster BranchandCut()
        {
            ...
            //bdmaster.model.ExportModel("bendersmaster.lp");
            if (data.subsetvalidcut)
            {
                generatesubsetvalidcuts(bdmaster);
            }
            ...
            //bdmaster.model.SetParam(Cplex.Param.MIP.Tolerances.MIPGap, 1e-6);
            // Turn on traditional search for use with control callbacks
            ...
            if (data.cuttingPlaneStrategy)
            {
                bdmaster.model.Use(new BendersUserCutCallback(bdmaster, data));
            }
        ...
        {
            #region
            Program.SolutionIteration.WriteLine("findworsttime, subproblemtime, multicutstime, paretotime, worst_scenario_cost, objective cost, bestbound, Omega");
            Program.g_comparativeIndicators.WriteLine("# feasol, # optsol, best integer, best bound, Gap, CPU time");
            ...
            //ccgmaster.model.Use(new BLCCUserCutforCCGMaster(ccgmaster, data));
        ...
        /// of branch and check scheme when facing a new schenario
        /// </summary>
        public CCGMaster BranchandCheckforCCGroot(CCGMaster ccgmaster)
        {
            #region
            Program.SolutionIteration.WriteLine("findworsttime, subproblemtime, multicutstime, paretotime, worst_scenario_cost, objective cost, bestbound, Omega");
            Program.g_comparativeIndicators.WriteLine("# feasol, # optsol, best integer, best bound, Gap, CPU time");
            ...
            // set time limitaion
            ccgmaster.model.SetParam(Cplex.Param.TimeLimit, 10);
            // Set up the cut callback to be used for separating Benders' cuts
            ccgmaster.model.SetParam(Cplex.Param.Preprocessing.Presolve, false);
            ccgmaster.model.SetParam(Cplex.Param.Threads, 1);
            ...
            //Turn on traditional search for use with control callbacks
            ccgmaster.model.SetParam(Cplex.Param.MIP.Strategy.Search, Cplex.MIPSearch.Traditional);
            ccgmaster.model.Use(new BendersforrootofCCGMaster(ccgmaster, data));
    ...
    class CandCG
    {
        ...
        public void ParallelCCG()
        {
            ...
            //for (int j = 0; j < data.DCSize; j++)
            //{
            //    if(j == 0 || j == 6 || j == 8 || j == 10 || j == 11 || j == 12 || j ==24 || j ==26 || j ==30)
            //        y_solution[j] = 1;
            //    else
            //        y_solution[j] = 0;
            //}
        ...
        public void CCGwBAC()
        {
            ...
            //CCGMaster ccgm = new CCGMaster();
            //ccgm.InitializeCCGMaster();

            CACGBD colBDGen = new CACGBD();
            //colBDGen.BranchandCheckforCCGroot(ccgm);

            ////add optimality cuts            
            //for (int c = 0; c < masterproblem.cutsStore.Count; c++)
            //{
            //    var sval = ccgm.model.GetSlack(ccgm.cutsStore[c]);
            //    Console.WriteLine(sval);
            //    masterproblem.model.AddCut(ccgm.cutsStore[c]);
            //}
            ////add feasi cuts            
            //for (int c = 0; c < masterproblem.feasiblecutsStore.Count; c++)
            //{
            //    masterproblem.model.AddCut(ccgm.feasiblecutsStore[c]);
            //}
            //masterproblem.cutsStore.Clear(); masterproblem.feasiblecutsStore.Clear();
            int number_of_opens = 0;
            for (int j = 0; j < data.DCSize; j++)
            {
            ...
            while (UB - LB > eps * UB && totalrunningtime <= data.TL)
            {
                ...
                if (cuttingflaglist.Count != 0)// existing a worst scenario
                {
                    ...
                    if (data.validcut2)
                    {
                        ILinearNumExpr validcut_2 = masterproblem.model.LinearNumExpr();
                        for (int j = 0; j < data.DCSize; j++)
                        {
                            int corCommunityNode = Program.g_DCent_list[j].CorNodeSeqNo;
                            double totalsupply = data.h_i[corCommunityNode];
                            ...
                            validcut_2.AddTerm((1 - worst_u_j[j]) * totalsupply * (data.c_j[j] + allocationcost), masterproblem.y_j[j]);

                        }
                        validcut_2.AddTerm(-1, masterproblem.omega);
                        masterproblem.model.AddGe(validcut_2, 0);
                    }
                }
                else//now no feasible solutions
                {
                    ...
                    if (sub_status == Cplex.Status.Optimal || sub_status == Cplex.Status.Feasible)
                    {
                        ...
                        if (data.solveCCGmasterIter)
                        {
                            if (subproblemscenarios.Count < data.maxMultiCuts)
                            {
                                ...
                                cuttingflaglist.Clear();
                            }
                            else
                            {
                                //First case: 
                                int[] temp_worst_u = subproblemscenarios[0];
                                subproblemscenarios.RemoveAt(0);
                                subproblemscenarios.Add(worst_u_j);

                                worst_u_j = temp_worst_u;
        ...
        public void CCGMILP()
        {
            ...
            //Cplex.Status dual_status = dualsubproblem.model.GetStatus();
            //if (dual_status == Cplex.Status.Optimal || dual_status == Cplex.Status.Feasible)
            //{
            //    feasible_flag = true;
            //}            
            //else if(dual_status == Cplex.Status.Infeasible)
            //{
            //    Console.WriteLine("Please check!!! No feasible solutions available!!!!");
            //    totalrunningtime = data.TL + 1;
            //}
            ...
            while (UB - LB > eps * UB && totalrunningtime <= data.TL)
            {
                ...
                //dualsubproblem.model.ExportModel("dualsubproblemmodel.lp");
                dualsubproblem.model.Solve();
                double dualccgsubobj = dualsubproblem.model.GetObjValue();
                        ...
                        if (data.multiCutStrategy)
                        {
                            ...
                            if (numSols >= 1)
                            {
                                ...
                                while (true)
                                {
                                    ...
                                    subproblem.ResetOBJaRHS_XI(y_solution, candidatesolset[ind]);
