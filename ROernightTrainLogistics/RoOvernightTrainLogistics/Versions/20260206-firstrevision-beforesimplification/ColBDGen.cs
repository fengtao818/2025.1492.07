...
namespace RoOvernightTrainLogistics
{
    ...
    internal class BendersLazyConsCallback : Cplex.LazyConstraintCallback
    {
        ...
        public BendersLazyConsCallback(BendersMaster Bendmaster, Data dt)
        {
            ...
            //ccgsub.model.SetParam(Cplex.Param.RootAlgorithm, Cplex.Algorithm.Primal);
    ...
    class CCGSub
    {
        ...
        /// <summary>
        /// Update model at each iteratio for simulation
        /// </summary>
        /// <param name="y_solution"></param-difference>        
        /// <param name="u_sol"></param-difference>
        /// <param name="demand"></param-difference>
        ...
        /// </summary>
        /// <param name="BDM"></param master problem>
        /// <param name="u_sol"></param worst scenario>
        /// <param name="y_sol"></param y solution>
        /// <returns></returns>
        ...
        public void ArcFLowModel1(int[] y_sol, int[] u_sol)
        {
            #region
            model = new Cplex();
            //对于小规模问题可加速,大规模系数问题效率不高,实测可在25*15*4算例中加速
            ...
            number_of_var = 0; number_of_con = 0;
            INumVar[,] f = new INumVar[data.DCSize, data.linkSize];
            INumVar[] s = new INumVar[data.DCSize];
            z_j = new INumVar[data.DCSize];

            for (int j = 0; j < data.DCSize; j++)
            {
                z_j[j] = model.NumVar(0, double.MaxValue,NumVarType.Float, $"z_{j}");
                s[j] = model.NumVar(double.MinValue, double.MaxValue, NumVarType.Float, $"s_{j}");
                for (int a = 0; a < data.linkSize; a++)
            ...
            //calculate the supply at each warehouse station
            for (int j = 0; j < data.DCSize; j++)
            {
            ...
            for (int j = 0; j < data.DCSize; j++)
            {
                ...
                model.AddLe(s[j], rightexpr, $"SupplyInjection_{j}");
            }
        ...
        public void ArcFLowModel(int[] y_sol, int[] u_sol)
        {
            ...
            for (int j = 0; j < data.DCSize; j++)
            {
                ...
                //model.AddLe(l_j[j], s[j], $"complementary_{j}");
            }
    ...
    class CACGBD
    {
        ...
        public BendersMaster BranchandCut()
        {
            ...
            // 1. 设置工作内存上限为 30GB (30 * 1024， 48 * 1024)，因电脑而异
            //bdmaster.model.SetParam(Cplex.Param.WorkMem, 30720);

            // 2. 极为关键：当超过 48GB 时，将节点存入硬盘并压缩
            // 这样即便模型极其复杂，它也会写到 SSD 缓存，而不是卡死系统
            //bdmaster.model.SetParam(Cplex.Param.MIP.Strategy.File, 3);

            // 3. 内存强调模式：这会让 CPLEX 在内部数据结构上更“抠门”
            //bdmaster.model.SetParam(Cplex.Param.Emphasis.Memory, true);
    ...
    class Simulation
    {
        ...
        public void TwoStgEva()
        {
            ...
            int realcountperdistr = data.realizationlist.Count / 3;
            ...
            double[] totaldemand = new double[data.distributionSize];//all demand for each distribution
            ...
            for (int d = 0; d < data.realizationlist.Count; d++)
            {
                ...
                {
                    double evacost = -10000/*Evaluation(y_sol, z_sol, r_sol, x_sol, s_sol)*/;
                    probrealizations[real.distributionSeqno, realInd] = evacost;
                    //record each underfulfill demand
            ...
            int[] indexarr = new int[data.distributionSize];//record index of the VaR95
        ...
        {            
            ...
            for (int d = 0; d < data.realizationlist.Count; d++)
            {
                ...
                int realInd = real.sceseqno - real.distributionSeqno * realcountperdistr;
                //demand = data.demandlist[realInd];//demand uncertainty
                demand = data.h_i;
                totaldemand[real.distributionSeqno] += demand.Sum();
                ...
                if (subpro.model.Solve())
                {
                    ...
                    //double objval = subpro.model.GetObjValue();
                    //pathsubpro.model.Solve();
                    //double objval2 = pathsubpro.model.GetObjValue();

                    //solution.write_NFMsolution(subpro, y_sol, real.scenario);
                    //Program.solution_output.Flush(); Program.solution_output.Close();
